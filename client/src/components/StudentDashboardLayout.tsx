...
export default function StudentDashboardLayout({
  ...
  const activeMenuItem = menuItems.find(item => 
    location === item.path || (item.path !== '/student' && location.startsWith(item.path))
  ) || menuItems[0];

  // Check if we're on a sub-page (not the main dashboard)
  const isSubPage = location !== '/student';
      ...
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-indigo-950">
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl">
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              This portal is for enrolled students only.
            </p>
          </div>
    ...
    <nav className="flex-1 overflow-y-auto py-2 px-2">
      {menuItems.map(item => {
        ...
        return (
          ...
          >
            <item.icon className={`h-4 w-4 shrink-0 ${isActive ? "text-blue-600 dark:text-blue-400" : ""}`} />
  ...
  const UserMenu = () => (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-3 rounded-lg px-2 py-2 hover:bg-blue-50 dark:hover:bg-slate-800 transition-colors w-full text-left">
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuItem onClick={() => setLocation("/student/profile")} className="cursor-pointer">
  ...
  if (isMobile) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50/50 to-indigo-50/50 dark:from-slate-950 dark:to-slate-900">
        {/* Mobile Header */}
        <header className="sticky top-0 z-50 flex h-14 items-center justify-between bg-white dark:bg-slate-900 border-b border-blue-100 dark:border-slate-700 px-2">
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium text-blue-900 dark:text-white text-sm">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="rounded-full">
                  <Avatar className="h-8 w-8 border border-blue-200 dark:border-slate-700">
                    <AvatarFallback className="text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                      {user?.name?.charAt(0).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
        ...
        <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
          <SheetContent side="left" className="w-72 p-0">
            ...
            <div className="flex flex-col h-full bg-white dark:bg-slate-900">
              <div className="h-14 flex items-center justify-between px-4 border-b border-blue-100 dark:border-slate-700">
                <span className="font-bold text-blue-900 dark:text-white">Student Portal</span>
              ...
              <div className="p-3 border-t border-blue-100 dark:border-slate-700">
                <UserMenu />
              </div>
            </div>
          </SheetContent>
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-blue-50/50 to-indigo-50/50 dark:from-slate-950 dark:to-slate-900">
      {/* Desktop Sidebar */}
      <aside 
        className="fixed inset-y-0 left-0 z-30 flex flex-col bg-white dark:bg-slate-900 border-r border-blue-100 dark:border-slate-700 transition-all duration-200"
        style={{ width: sidebarWidth }}
      >
        <div className="h-14 flex items-center justify-between px-3 border-b border-blue-100 dark:border-slate-700">
          {!sidebarCollapsed && (
            <span className="font-bold text-blue-900 dark:text-white">Student Portal</span>
          )}
          <button
            onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
            className="h-8 w-8 flex items-center justify-center hover:bg-blue-50 dark:hover:bg-slate-800 rounded-lg transition-colors"
          >
        ...
        <div className="px-2 pt-2 space-y-2">
          <Button
            ...
            size="sm"
            onClick={() => setLocation("/")}
            className={`w-full justify-start gap-2 border-blue-200 dark:border-slate-700 ${sidebarCollapsed ? 'px-2' : ''}`}
          >
