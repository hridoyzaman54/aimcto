...
class CalmingSynthesizer {
  ...
  // Ocean wave synthesis using filtered white noise
  playOcean() {
    this.initCtx();
    this.stopAll();
    if (!this.ctx) return;
    ...
    // Filter to shape noise into soft waves
    const filter = this.ctx.createBiquadFilter();
    filter.type = 'lowpass';
    filter.Q.value = 1.0;

    // LFO to modulate filter frequency (simulating ocean wave rise and fall)
    const lfo = this.ctx.createOscillator();
    lfo.frequency.value = 0.08; // Super slow wave cycle (12 seconds)
    ...
    gain.gain.value = 0.001; // Start soft
  ...
  playWind() {
    ...
    const lfo = this.ctx.createOscillator();
    lfo.frequency.value = 0.15; // Slow wind gust modulation
  ...
  playHarp() {
    ...
    const scheduleNextNote = () => {
      ...
      // Move to next note in a relaxing pentatonic climb/descent
      const step = Math.random() > 0.5 ? 1 : -1;
      currentNoteIndex = (currentNoteIndex + step + pentatonicScale.length) % pentatonicScale.length;

      // Schedule next note in 1.2 to 1.8 seconds (organic tempo)
  ...
  playBirds() {
    ...
    const scheduleBirdChirp = () => {
      ...
      // Random delay simulating natural bird calls
...
function InteractiveShatterZone() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw instruction background text
    ...
    const floater = setInterval(() => {
      setBalloons(prev =>
        prev
          ...
          .filter(b => b.y > -20) // remove offscreen
      );
    }, 30);
...
function MemorySoundGame() {
  ...
  const notes = [261.63, 293.66, 329.63, 349.23, 392.00, 440.00]; // Pentatonic notes
...
// ==========================================
// 6. SPECIAL NEEDS COURSES INFINITE CAROUSEL
// ==========================================
interface Course {
  id: number;
  title: string;
  ...
  return (
    <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 md:p-12 shadow-2xl relative">
      <div className="flex flex-col md:flex-row gap-8 items-center justify-between">
        ...
        {/* Left Side: Course Image and controls */}
...
// ==========================================
// 8. INTERACTIVE AUTISM LEVELS SECTION
// ==========================================
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground selection:bg-primary/20 selection:text-primary">
      ...
      <main className="w-full overflow-hidden">
        ...
        <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-blue-50 via-indigo-50 to-emerald-50 dark:from-background dark:via-secondary/10 dark:to-background border-b border-border/30">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {[...Array(8)].map((_, i) => (
              <motion.div
                ...
                className="absolute rounded-full opacity-20 dark:opacity-10 blur-[1px]"
                style={{
                  width: 80 + i * 50,
                  height: 80 + i * 50,
                  left: `${5 + i * 12}%`,
                  top: `${15 + (i % 3) * 25}%`,
                  background: `radial-gradient(circle, hsl(${210 + i * 25}, 80%, 75%) 0%, transparent 70%)`
                }}
              />
            ))}
          </div>
          ...
          <div className="container relative z-10 py-16 sm:py-24">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-16 items-center">
              ...
              >
                ...
                <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-[1.05] tracking-tight">
                ...
                <p className="text-sm sm:text-base md:text-lg text-muted-foreground leading-relaxed max-w-lg mx-auto lg:mx-0">
                  Welcome to AIM Centre's specialized sensory harbor. Through interactive sound therapy, colorful particle stimulation, and customized Autism Level 1, 2, and 3 programs, we help neurodiverse children express their beautiful potentials.
                </p>
        ...
        {/* === SENSORY ZONE === */}
        <section className="py-16 sm:py-24 bg-[#EFEDE8] dark:bg-card/30 border-b border-border/30">
        ...
        {/* === GAMES ZONE === */}
        <section className="py-16 sm:py-24 bg-background border-b border-border/30">
          <div className="container">
            <motion.div {...fadeUp} className="text-center mb-12 sm:mb-16 space-y-4">
